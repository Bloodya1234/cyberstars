...
export default function ConnectDiscordPage() {
  ...
  const [token, setToken] = useState(null);      // Custom token (опционально для state)
  const [loading, setLoading] = useState(true);
  const [errorText, setErrorText] = useState('');

  // Проверяем текущую сессию (по session cookie)
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        const me = await meRes.json(); // { uid: 'steam:7656...', ... }

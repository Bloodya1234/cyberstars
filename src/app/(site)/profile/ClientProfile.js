  ...
  return (
    <ClientLayout>
      <div className="min-h-screen bg-background text-primary">
        <EligibilityModal show={showModal} onClose={() => setShowModal(false)} t={t} />
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {languageOptions.map((lang) => (
                    <button
                      ...
                      className={`hex-button text-sm ${user.languages?.includes(lang) ? 'active-cyan' : ''}`}
                    >
                      {lang}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {positionOptions.map((pos) => (
                    <button
                      ...
                      className={`hex-button text-sm ${preferredPositions.includes(pos) ? 'active-green' : ''}`}
                    >
                      {pos}
                    </button>
                  ))}
